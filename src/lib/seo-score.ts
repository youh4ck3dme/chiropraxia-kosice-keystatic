...
export function computeSeoScore(input: SeoInputs): SeoScoreResult {
  ...
  if (body) {
    if (title && body.includes(title)) {
      recommendations.push({
        ...
        type: 'warning',
        message:
          'Kontrola duplicity: meta title sa vyskytuje v obsahu článku. Skontrolujte, či to nie je duplicita.',
      });
    }
    if (desc && body.includes(desc)) {
      recommendations.push({
        ...
        type: 'warning',
        message:
          'Kontrola duplicity: meta description sa vyskytuje v obsahu. Vyhnite sa duplicitným meta tagom.',
      });
    }
    ...
    if (noAlt.count > 0) {
      recommendations.push({
        id: 'img-alt-missing',
        type: 'warning',
        message: `Pridajte ALT text k obrázku (${noAlt.count} obrázok/ov bez ALT v obsahu).`,
      });
    }
  }
  ...
  if (titleLen >= TITLE_IDEAL_MIN && titleLen <= TITLE_IDEAL_MAX) {
    ...
    recommendations.push({
      id: 'title-length',
      type: 'success',
      message: `Dĺžka meta title (${titleLen}) je v odporúčanom rozsahu 50–60 znakov.`,
    });
    ...
    if (titleLen < TITLE_IDEAL_MIN) {
      recommendations.push({
        id: 'title-short',
        type: 'warning',
        message: `Meta title má ${titleLen} znakov. Odporúčané: 50–60 znakov.`,
      });
    } else {
      recommendations.push({
        id: 'title-long',
    ...
    if (titleLen < TITLE_ABS_MIN) {
      recommendations.push({
        id: 'title-too-short',
        type: 'error',
        message: `Meta title je príliš krátky (${titleLen} zn.). Odporúčané aspoň 30, ideálne 50–60.`,
      });
    } else if (titleLen > TITLE_ABS_MAX) {
      recommendations.push({
        id: 'title-too-long',
        type: 'error',
        message: `Meta title je príliš dlhý (${titleLen} zn.). Google zvykne skrátiť nad 70 znakov.`,
      });
    }
  }
  ...
  if (descLen >= DESC_IDEAL_MIN && descLen <= DESC_IDEAL_MAX) {
    ...
    recommendations.push({
      id: 'desc-length',
      type: 'success',
      message: `Dĺžka meta description (${descLen}) je v odporúčanom rozsahu 120–160 znakov.`,
    });
    ...
    if (descLen < DESC_IDEAL_MIN) {
      recommendations.push({
        id: 'desc-short',
        type: 'warning',
        message: `Meta description má ${descLen} znakov. Odporúčané: 120–160 znakov.`,
      });
    } else {
      recommendations.push({
        id: 'desc-long',
    ...
    if (descLen < DESC_ABS_MIN) {
      recommendations.push({
        id: 'desc-too-short',
        type: 'error',
        message: `Meta description je príliš krátka (${descLen} zn.). Odporúčané 120–160 znakov.`,
      });
    } else if (descLen > DESC_ABS_MAX) {
      recommendations.push({
        id: 'desc-too-long',
        type: 'error',
        message: `Meta description je príliš dlhá (${descLen} zn.). Google zvykne skrátiť nad 160 znakov.`,
      });
    }
  }
  ...
  if (focus) {
    if (focusInTitle) {
      ...
      recommendations.push({
        id: 'focus-title',
        type: 'success',
        message: `Kľúčové slovo „${focus}“ je v meta title.`,
      });
    } else {
      recommendations.push({
        id: 'focus-title-missing',
        type: 'warning',
        message: `Pridajte kľúčové slovo „${focus}“ do meta title (alebo názvu článku).`,
      });
    }
    if (focusInDesc) {
      ...
      recommendations.push({
        id: 'focus-desc',
        type: 'success',
        message: `Kľúčové slovo „${focus}“ je v meta description.`,
      });
    } else {
      recommendations.push({
        id: 'focus-desc-missing',
        type: 'warning',
        message: `Pridajte kľúčové slovo „${focus}“ do meta description.`,
      });
    }
    if (focusInSlug) {
    ...
    recommendations.push({
      id: 'focus-missing',
      type: 'warning',
      message: 'Vyplňte Focus Keyword pre lepšie SEO.',
    });
  }
  ...
  if (input.coverImageAlt && input.coverImageAlt.trim().length > 0) {
    ...
    recommendations.push({
      id: 'alt-ok',
      type: 'success',
      message: 'Titulný obrázok má vyplnený ALT text.',
    });
  } else {
    recommendations.push({
      id: 'alt-missing',
      type: 'warning',
      message: 'Vyplňte ALT text titulného obrázka pre prístupnosť a SEO.',
    });
  }
  ...
  if (body && focus) {
    ...
    if (keywordOccurrences.count > 0) {
      recommendations.push({
        id: 'keyword-in-body',
        type: 'success',
        message: `Kľúčové slovo „${focus}“ sa v texte vyskytuje ${keywordOccurrences.count}×.`,
      });
    } else {
      recommendations.push({
        id: 'keyword-not-in-body',
        type: 'warning',
        message: `Kľúčové slovo „${focus}“ sa v texte nevyskytuje. Zvážte jeho použitie v odstavcoch.`,
      });
    }
  }
