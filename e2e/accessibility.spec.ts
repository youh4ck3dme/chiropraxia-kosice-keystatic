...
test.describe('Accessibility & UI/UX', () => {
  ...
  test('booking flow page accessibility', async ({ page }) => {
    ...
    await page.route('**/rest/v1/services*', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify([
          { id: '1', name: 'Vstupné vyšetrenie', description: 'Komplexné vyšetrenie', duration_min: 30, price: 35, sort_order: 1 }
        ])
      });
    });
  ...
  test('visual contrast check (manual hint)', async ({ page }) => {
    ...
    await expect(page.locator('h1').filter({ hasText: /Zbavte sa|bolesti/i }).first()).toBeVisible();
