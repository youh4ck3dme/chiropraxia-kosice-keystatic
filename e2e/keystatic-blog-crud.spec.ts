...
const testDescription =
  'Krátky SEO popis pre E2E test. Toto je meta description medzi 120 a 160 znakov pre overenie validácie a zobrazenia na webe.';
...
test.describe('Keystatic blog CRUD', () => {
  ...
  test('create new blog entry: form fields accept required data', async ({ page }) => {
    ...
    await expect(titleInput.or(page.locator('input[name*="title"], [data-label*="Názov"]').first()))
    ...
    const slugField = page
      .locator('input[name*="slug"], input[placeholder*="slug"], [data-field*="slug"]')
      .first();
  ...
  test('edit existing blog entry: list shows entries and edit opens without error', async ({
    page,
  }) => {
    await page.goto('/keystatic', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2000);
  ...
  test('after saving new entry, meta title and description are reflected on public blog post page', async ({
    page,
  }) => {
    await page.goto('/keystatic', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(2000);
